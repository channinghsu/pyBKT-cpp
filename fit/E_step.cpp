...
static PyObject *run(PyObject *module, PyObject *args) {
    ...
    PyObject *data_ptr = NULL, *model_ptr = NULL, *t_softcounts = NULL,
             *e_softcounts = NULL, *i_softcounts = NULL, *fixed = NULL;
    PyArrayObject *t_softcounts_np = NULL, *e_softcounts_np = NULL,
                  *i_softcounts_np = NULL, *alldata = NULL,
                  *allresources = NULL, *starts = NULL, *lengths = NULL,
                  *learns = NULL, *forgets = NULL, *guesses = NULL,
                  *slips = NULL; // Extended Numpy/C API
    int num_outputs, parallel, isMultigs, isCognitiveLabel;
    double prior;
    // dict& data, dict& model, numpy::ndarray& trans_softcounts,
    // numpy::ndarray& emission_softcounts, numpy::ndarray& init_softcounts, int
    // num_outputs

    // "O" format -> read argument as a PyObject type into argy (Python/C API)
    if (!PyArg_ParseTuple(args, "OOiiOii", &data_ptr, &model_ptr, &num_outputs,
                          &parallel, &fixed, &isMultigs, &isCognitiveLabel)) {
    ...
    char *DM_NAMES[] = {"data",   "resources", "starts",  "lengths",
                        "learns", "forgets",   "guesses", "slips"};
    PyArrayObject **DM_PTRS[] = {&alldata, &allresources, &starts,  &lengths,
                                 &learns,  &forgets,      &guesses, &slips};
    for (int i = 0; i < 8; i++) {
        PyObject *dp =
            PyDict_GetItemString(i < 4 ? data_ptr : model_ptr, DM_NAMES[i]);
        int DTYPE = PyArray_ObjectType(
            dp, (i < 4 ? (i < 1 ? NPY_INT : NPY_INT64)
                       : NPY_FLOAT)); // hack to force correct type
        *DM_PTRS[i] =
            (PyArrayObject *)PyArray_FROM_OTF(dp, DTYPE, NPY_ARRAY_IN_ARRAY);
    }
    prior = PyFloat_AsDouble(PyDict_GetItemString(model_ptr, "prior"));

    int bigT = (int)PyArray_DIM(alldata, 1),
        num_subparts = (int)PyArray_DIM(alldata, 0);

    if (isMultigs && isCognitiveLabel) {
        num_subparts = num_subparts * 2;
    }
    ...
    Map<Array<int32_t, Dynamic, Dynamic, RowMajor>, Aligned> alldata_arr(
        reinterpret_cast<int *>(PyArray_DATA(alldata)), num_subparts, bigT);
    Map<Array<int64_t, Eigen::Dynamic, 1>, Aligned> allresources_arr(
        reinterpret_cast<int64_t *>(PyArray_DATA(allresources)),
        len_allresources, 1);
    Map<Array<int64_t, Eigen::Dynamic, 1>, Aligned> starts_arr(
        reinterpret_cast<int64_t *>(PyArray_DATA(starts)), num_sequences, 1);
    Map<Array<int64_t, Eigen::Dynamic, 1>, Aligned> lengths_arr(
        reinterpret_cast<int64_t *>(PyArray_DATA(lengths)), len_lengths, 1);
    ...
    // TODO: NEED TO FIX THIS I'M CREATING NEW ARRAYS AND I NEED TO USE THE
    // ARGUMENTS!!!
    // TODO: FIX THIS!!!
    /*Map<ArrayXXd,Aligned>
    all_trans_softcounts(trans_softcounts,2,2*num_resources);
    all_trans_softcounts.setZero();
    Map<Array2Xd,Aligned>
    all_emission_softcounts(emission_softcounts,2,2*num_subparts);
    ...
    all_emission_softcounts.setZero();
    Array2d all_initial_softcounts(2, 1); //should i use these dimensions?
    the same as the original vector?? all_initial_softcounts.setZero();*/
    ...
    Map<ArrayXXd, Aligned> all_trans_softcounts(r_trans_softcounts, 2,
                                                2 * num_resources);
    all_trans_softcounts.setZero();
    Map<Array2Xd, Aligned> all_emission_softcounts(r_emission_softcounts, 2,
                                                   2 * num_subparts);
    all_emission_softcounts.setZero();
    Map<Array2d, Aligned> all_initial_softcounts(r_init_softcounts);
    all_initial_softcounts.setZero();

    // TODO: FIX THIS!!! I'll replace all these weird arrays for zeroes
    // ones. Array2Xd likelihoods_out(2,bigT); likelihoods_out.setZero();
    ...
    /* switch (num_outputs)
    {
        case 4:
            ...
            new (&likelihoods_out)
    Map<Array2Xd,Aligned>(mxGetPr(plhs[3]),2,bigT); case 3: plhs[2] =
    mxCreateDoubleMatrix(2,bigT,mxREAL); new (&gamma_out)
    Map<Array2Xd,Aligned>(mxGetPr(plhs[2]),2,bigT); case 2: plhs[1] =
    mxCreateDoubleMatrix(2,bigT,mxREAL); new (&alpha_out)
    Map<Array2Xd,Aligned>(mxGetPr(plhs[1]),2,bigT); case 1: plhs[0] =
    mxCreateDoubleScalar(0.); total_loglike = mxGetPr(plhs[0]);
    }*/
...
#pragma omp parallel
    {
        double s_trans_softcounts[2 * 2 * num_resources]
            __attribute__((aligned(16)));
        double s_emission_softcounts[2 * 2 * num_subparts]
            __attribute__((aligned(16)));
        Map<ArrayXXd, Aligned> trans_softcounts_temp(s_trans_softcounts, 2,
                                                     2 * num_resources);
        Map<ArrayXXd, Aligned> emission_softcounts_temp(s_emission_softcounts,
                                                        2, 2 * num_subparts);
        ...
        int sequence_idx_end =
            min(sequence_idx_start + blocklen, num_sequences);
        // mexPrintf("start:%d   end:%d\n", sequence_idx_start,
        // sequence_idx_end);

        for (int sequence_index = sequence_idx_start;
             sequence_index < sequence_idx_end; sequence_index++) {
            ...
            for (int t = 0; t < T - 1; t++) {
                int64_t resources_temp =
                    allresources_arr(sequence_start + t, 0);
                alpha.col(t + 1) =
                    (As.block(0, 2 * (resources_temp - 1), 2, 2) * alpha.col(t))
                        .array() *
                    likelihoods.col(t + 1);
            ...
            for (int n = 0; n < num_subparts; n++) {
                ...
                if (data_temp != 0) {
                    emission_softcounts_temp.col(2 * n + (data_temp == 2)) +=
                        gamma.col(T - 1);
                }
            }
            ...
            for (int t = T - 2; t >= 0; t--) {
                ...
                trans_softcounts_temp.block(0, 2 * (resources_temp - 1), 2,
                                            2) += pair;
                ...
                for (int n = 0; n < num_subparts; n++) {
                    int32_t data_temp = alldata_arr(n, sequence_start + t);
                    if (data_temp != 0) {
                        emission_softcounts_temp.col(
                            2 * n + (data_temp == 2)) += gamma.col(t);
                    }
                }
            }
            ...
            {
                case 4:
                    likelihoods_out.block(0,sequence_start,2,T) =
            likelihoods; case 3: gamma_out.block(0,sequence_start,2,T) =
            gamma; case 2: alpha_out.block(0,sequence_start,2,T) = alpha;
            } */
...
#pragma omp critical
    ...
    PyObject *all_trans_softcounts_arr = (PyObject *)PyArray_SimpleNewFromData(
        3, dims1, NPY_DOUBLE, r_trans_softcounts);
    PyObject *capsule1 =
        PyCapsule_New(r_trans_softcounts, NULL, capsule_cleanup);
    PyArray_SetBaseObject((PyArrayObject *)all_trans_softcounts_arr, capsule1);

    npy_intp dims2[] = {num_subparts, 2, 2};
    PyObject *all_emission_softcounts_arr =
        (PyObject *)PyArray_SimpleNewFromData(3, dims2, NPY_DOUBLE,
                                              r_emission_softcounts);
    PyObject *capsule2 =
        PyCapsule_New(r_emission_softcounts, NULL, capsule_cleanup);
    PyArray_SetBaseObject((PyArrayObject *)all_emission_softcounts_arr,
                          capsule2);

    npy_intp dims3[] = {2, 1};
    PyObject *all_initial_softcounts_arr =
        (PyObject *)PyArray_SimpleNewFromData(2, dims3, NPY_DOUBLE,
                                              r_init_softcounts);
    PyObject *capsule3 =
        PyCapsule_New(r_init_softcounts, NULL, capsule_cleanup);
    PyArray_SetBaseObject((PyArrayObject *)all_initial_softcounts_arr,
                          capsule3);
    ...
    PyDict_SetItemString(result, "all_trans_softcounts",
                         all_trans_softcounts_arr);
    PyDict_SetItemString(result, "all_emission_softcounts",
                         all_emission_softcounts_arr);
    PyDict_SetItemString(result, "all_initial_softcounts",
                         all_initial_softcounts_arr);
    PyDict_SetItemString(result, "alpha", alpha_out_arr);
    PyDict_SetItemString(result, "total_loglike",
                         PyLong_FromLong(*total_loglike));
...
static struct PyModuleDef E_step_module = {
    PyModuleDef_HEAD_INIT, "E_step", /* name of module */
    NULL,                            /* module documentation, may be NULL */
    -1, /* size of per-interpreter state of the module,
           or -1 if the module keeps state in global variables. */
    E_step_Methods};

PyMODINIT_FUNC PyInit_E_step() { return PyModule_Create(&E_step_module); }
