...
static PyObject *run(PyObject *module, PyObject *args) {
    ...
    PyArrayObject *alldata = NULL, *allresources = NULL, *starts = NULL,
                  *lengths = NULL, *learns = NULL, *forgets = NULL,
                  *guesses = NULL, *slips = NULL, *forward_messages = NULL;
    double prior;
    int parallel, skillNum, isMultigs, isCognitiveLabel;

    if (!PyArg_ParseTuple(args, "OOOiiii", &data_ptr, &model_ptr, &fwd_msgs,
                          &parallel, &skillNum, &isMultigs, &isCognitiveLabel)) {
    ...
    forward_messages =
        (PyArrayObject *)PyArray_FROM_OTF(fwd_msgs, DTYPE, NPY_ARRAY_IN_ARRAY);
    ...
    // forward messages
    // numpy::ndarray all_forward_messages =
    // extract<numpy::ndarray>(forward_messages);
    double *forward_messages_temp = new double[2 * bigT];
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < bigT; j++) {
            forward_messages_temp[i * bigT + j] =
                extract_double_2d(forward_messages, i, j);
        }
    }
...
#pragma omp parallel for
    for (int sequence_index = 0; sequence_index < num_sequences;
         sequence_index++) {
        ...
        Map<MatrixXd, Aligned> forward_messages(
            forward_messages_temp + 2 * sequence_start, 2, T);
        Map<MatrixXd, Aligned> predictions(all_predictions + 2 * sequence_start,
                                           2, T);

        predictions.col(0) = initial_distn;
        for (int t = 0; t < T - 1; t++) {
            int64_t resources_temp =
                extract_int64_t(allresources, sequence_start + t);
            predictions.col(t + 1) =
                As.block(0, 2 * (resources_temp - 1), 2, 2) *
                forward_messages.col(t);
        }
    }

    npy_intp dims[] = {2, bigT};
    PyObject *all_predictions_arr = (PyObject *)PyArray_SimpleNewFromData(
        2, dims, NPY_DOUBLE, all_predictions);
...
static PyMethodDef predict_onestep_states_Methods[] = {
    {"run", run, METH_VARARGS, "Generates predictions for BKT model"},
    {NULL, NULL, 0, NULL} /* Sentinel */
};

static struct PyModuleDef predict_onestep_states_module = {
    PyModuleDef_HEAD_INIT, "predict_onestep_states", /* name of module */
