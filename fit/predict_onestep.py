...
def run(model, data, skillNum, parallel=True, fixed={}, isMultigs=False, isCognitiveLabel=False):
    ...
    if (isMultigs and isCognitiveLabel):
        num_subparts *= 2

    result = E_step.run(data, model, 1, int(parallel), fixed, int(isMultigs), int(isCognitiveLabel))
    ...
    state_predictions = predict_onestep_states.run( data, model, result["alpha"], int(parallel), int(skillNum), int(isMultigs), int(isCognitiveLabel))
    p = state_predictions.shape
    state_predictions = state_predictions.flatten(order="C")
    state_predictions = state_predictions.reshape(p, order="F")
