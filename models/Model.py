...
class Model:
    ...
    def partial_fit(self, data_path=None, data=None, **kwargs):
        ...
        self._update_param( ["skills", "num_fits", "defaults", "fixed", "parallel", "forgets"], kwargs)
        ...
        for skill in all_data:
            self.fit_model[skill] = self._fit( all_data[skill], skill, self.forgets, preload=kwargs["preload"] if "preload" in kwargs else False)
        self.manual_param_init = False
    ...
    def predict(self, data_path=None, data=None):
        ...
        for skill in all_data:
            correct_predictions, state_predictions = self._predict(
                self.fit_model[skill], all_data[skill], skillNum
            )
            skillNum += 1
            ...
            df.loc[all_data[skill]["index"], "correct_predictions"] = (
                correct_predictions
            )
    ...
    def evaluate(self, data=None, data_path=None, metric=metrics.rmse):
        ...
        all_data = self._data_helper( data_path, data, self.defaults, self.skills, self.model_type, gs_ref=self.fit_model, resource_ref=self.fit_model)
        ...
        if cognitive_label is not None:
            isCognitiveLabel = True
        else:
            isCognitiveLabel = False
        if self.model_type[Model.MODELS_BKT.index("multigs")]:
            isMultigs = True
        else:
            isMultigs = False

        results = self._evaluate(all_data, metric,isMultigs=isMultigs,isCognitiveLabel=isCognitiveLabel)
    ...
    def _data_helper( self, data_path, data, defaults, skills, model_type, gs_ref=None, resource_ref=None, return_df=False, folds=False):
        ...
        if isinstance(data_path, str):
            data_p = data_helper.convert_data( data_path, skills, defaults=defaults, model_type=model_type, gs_refs=gs_ref, resource_refs=resource_ref, return_df=return_df, folds=folds)
        elif isinstance(data, pd.DataFrame):
            data_p = data_helper.convert_data( data, skills, defaults=defaults, model_type=model_type, gs_refs=gs_ref, resource_refs=resource_ref, return_df=return_df, folds=folds)
        if not return_df:
    ...
    def _fit(self, data, skill, forgets, preload=False):
        ...
        for i in range(num_fit_initializations):
            ...
            if self.model_type[Model.MODELS_BKT.index("multigs")]:
                ...
                isMultigs = False
            fitmodel = random_model_uni.random_model_uni( num_learns, num_gs, cognitive_label=cognitive_label, multigs=isMultigs)
            optional_args = {"fixed": {}}
            if forgets:
            ...
            if not preload:
                fitmodel, log_likelihoods = EM_fit.EM_fit( fitmodel, data, parallel=self.parallel, **optional_args, isMultigs=isMultigs, isCognitiveLabel=isCognitiveLabel)
    ...
    def _predict(self, model, data, skillNum, isMultigs=False, isCognitiveLabel=False) :
        """Helper function for predicting."""
        return predict_onestep.run(model, data, skillNum, isMultigs=isMultigs,isCognitiveLabel=isCognitiveLabel)

    def _evaluate(self, all_data, metric, isMultigs=False, isCognitiveLabel=False):
        """Helper function for evaluating."""
        per_skill = []
        true, pred = [], []
        skillNum = 0
        for skill in all_data:
            correct_predictions, state_predictions = self._predict( self.fit_model[skill], all_data[skill], skillNum,isMultigs=isMultigs,isCognitiveLabel=isCognitiveLabel)
            skillNum += 1
